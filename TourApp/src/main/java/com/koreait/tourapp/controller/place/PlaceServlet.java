...
public class PlaceServlet extends HttpServlet{
	...
	//xml 문자열을 파싱하여 자바 객체로 변환한 후 다시 json으로 변환하고 전달
	public String getJson1(HttpURLConnection conn) {
		...
		try {
			...
			//자바의 객체를 json문자열로 변환하는 과정을 개발자가 일일히 처리할 필요가 없다..
...
//			str = gson.toJson(map);
	...
	public String getJson3(HttpURLConnection conn) {
		 ...
		 StringBuilder sb = new StringBuilder();
	        ...
	        JSONObject res=(JSONObject)json.get("response"); //response key 가 가리키는 json
	        JSONObject body=(JSONObject)res.get("body");//body key 가 가리키는 json 
